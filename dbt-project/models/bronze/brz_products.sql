{{
    config(
        ...
        tags=['zone:bronze', 'sensitivity:public', 'domain:product']
    )
}}

with source as (
    select * from {{ ref('trn_products') }}
...
final as (
    select
        ...
        'product'                                                               as _domain,
        'public'                                                                as _sensitivity_tag,
        current_timestamp()                                                     as _bronze_inserted_at,
        {{ dbt_utils.generate_surrogate_key(['product_id', '_batch_id']) }}     as _bronze_sk
    from source
)
